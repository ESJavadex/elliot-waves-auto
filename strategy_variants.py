...
class ElliotWaveStrategies:
    ...
    def conservative_elliott_wave(self, data, analysis, sr_levels, current_price):
        ...
        """
        signal = self._init_signal(current_price)
    ...
    def _init_signal(self, current_price=None):
        """Initialize signal structure with default values based on current price."""
        # Provide realistic default values even for no_trade signals
        default_entry = current_price if current_price else 100.0
        default_stop = default_entry * 0.95  # 5% default stop loss
        default_targets = [
            default_entry * 1.02,  # 2% target
            default_entry * 1.05,  # 5% target  
            default_entry * 1.08   # 8% target
        ]
        ...
        return {
            ...
            "risk_reward": 1.0,  # Default 1:1 ratio
            "position_size": 100,
            ...
            "account_size": 10000,
            "confidence_score": 0,
            "notes": "Strategy did not identify a valid trade setup - showing default levels"
        }
...
def create_signal_comparison_table(comparison_summary):
    ...
    for result in comparison_summary:
        ...
        # Always show entry price if available
        if result['entry']:
            entry = f"${result['entry']:.2f}"
        else:
            entry = "No Entry"
        
        # For no_trade signals, show clearer status
        if signal == 'NO_TRADE':
...
def create_risk_comparison_table(comparison_summary):
    ...
    for result in comparison_summary:
        ...
        # Show stop loss or indicate no trade
        if result['signal'] == 'no_trade':
            stop_loss = 'No Trade'
            risk_per_share = 'No Risk'
            rr_ratio = 'No Trade'
        else:
            stop_loss = f"${result['stop_loss']:.2f}" if result['stop_loss'] else 'Not Set'
            ...
            if result['entry'] and result['stop_loss']:
                ...
                risk_per_share = 'Cannot Calculate'
            
            # Risk-reward ratio
            rr_ratio = f"{result['risk_reward']:.2f}:1" if result['risk_reward'] else 'Not Set'
...
def create_target_comparison_table(comparison_summary):
    ...
    for result in comparison_summary:
        ...
        # Handle no_trade signals differently
        if result['signal'] == 'no_trade':
            target1 = target2 = target3 = 'No Trade'
            max_profit = 'No Trade'
        else:
            target1 = f"${targets[0]:.2f}" if len(targets) > 0 and targets[0] else 'Not Set'
            target2 = f"${targets[1]:.2f}" if len(targets) > 1 and targets[1] else 'Not Set'
            target3 = f"${targets[2]:.2f}" if len(targets) > 2 and targets[2] else 'Not Set'
            
            # Calculate max profit potential
            max_profit = 'Cannot Calculate'
